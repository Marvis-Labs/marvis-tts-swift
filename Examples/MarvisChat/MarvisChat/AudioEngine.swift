...
final class AudioEngine {
    ...
    func setup() throws {
        ...
        input.installTap(onBus: 0, bufferSize: inputBufferSize, format: nil) { [weak self] buf, _ in
            Task { [weak self] in self?.processInputBuffer(buf) }
        }
    ...
    private func convertOnce(
        ...
        _ = converter.convert(to: dstBuffer, error: &error) { _, outStatus in
            if let src = srcBuffer, !local.isEmpty {
    ...
    private func enqueue(_ buffer: AVAudioPCMBuffer) {
        ...
        streamingPlayer.scheduleBuffer(buffer, completionCallbackType: .dataConsumed) { [weak self] _ in
            guard let self else { return }
